"""Anonymous PlanetBids platform-family connector.

PlanetBids provides configurable agency-specific vendor portals. This family is
deliberately separate from VendorLine and every other procurement platform.
"""
...
class PlanetBidsError(RuntimeError):
    pass


class PlanetBidsAccessError(PlanetBidsError):
    def __init__(self, state: PlanetBidsAccessState):
        self.state = state
        super().__init__(f"PlanetBids access boundary: {state.value}")


@dataclass(frozen=True, slots=True)
class PlanetBidsProfile:
    profile_key: str
    jurisdiction: str
    state_code: str
    ...
    agency_page_url: str = ""
    discovery_url: str = ""
    supported_hostname: str = "www.planetbids.com"
...
FIXTURE_PROFILE = PlanetBidsProfile(
    ...
    agency_page_url="https://www.planetbids.com/app/agencies/fl/fixture-county",
    discovery_url="https://www.planetbids.com/app/agencies/fl/fixture-county/procurement-opportunities",
    detail_url_template="https://www.planetbids.com/app/agencies/fl/fixture-county/procurement-opportunities/{opportunity_id}",
...
@dataclass(frozen=True, slots=True)
class PlanetBidsQuery(ConnectorQuery):
...
def detect_access_boundary(text: str) -> PlanetBidsAccessState:
    value = re.sub(r"\s+", " ", text).casefold()
    for state, terms in (
        (
            PlanetBidsAccessState.INVITATION_ONLY,
            ("by invitation only", "invited vendors only", "invitation only"),
        ),
        (
            PlanetBidsAccessState.PROSPECTIVE_BIDDER_REQUIRED,
            (
                "prospective bidder",
        ...
        (
            PlanetBidsAccessState.REGISTRATION_REQUIRED,
            ("registration required", "create a supplier account", "register to download"),
        ),
        (
            PlanetBidsAccessState.LOGIN_REQUIRED,
            ("login required", "sign in to continue", "supplier login"),
        ),
        (PlanetBidsAccessState.RESTRICTED, ("access denied", "not authorized")),
        (PlanetBidsAccessState.UNAVAILABLE, ("temporarily unavailable", "scheduled maintenance")),
    ):
        if any(term in value for term in terms):
            return state
    return PlanetBidsAccessState.PUBLIC
...
class PlanetBidsConnector(BaseConnector):
    ...
    def __init__(
        self,
        profile: PlanetBidsProfile = FIXTURE_PROFILE,
        *,
        ...
        self._access = PlanetBidsAccessState.UNKNOWN
    ...
    async def discover(self, query: ConnectorQuery) -> AsyncIterator[RawOpportunity]:
        q = (
            query
            if isinstance(query, PlanetBidsQuery)
            else PlanetBidsQuery(
        ...
        if self.profile.profile_status in {"migrated", "legacy", "unavailable"}:
            raise PlanetBidsAccessError(
                PlanetBidsAccessState.MIGRATED
                if self.profile.profile_status != "unavailable"
                else PlanetBidsAccessState.UNAVAILABLE
            )
        if not self._configuration_valid():
            raise PlanetBidsError("invalid or unsafe PlanetBids profile")
        ...
        for page in range(1, pages + 1):
            ...
            if boundary is not PlanetBidsAccessState.PUBLIC:
                raise PlanetBidsAccessError(boundary)
            items, links = parse_page(response.text)
            if not items:
                if page == 1 and not re.search(
                    r"no (?:open )?(?:opportunities|solicitations)|data-planetbids-list",
                    response.text,
                    re.I,
                ):
                    raise PlanetBidsAccessError(PlanetBidsAccessState.CHANGED_MARKUP)
                break
            for data in items:
                raw_id = (
        ...
        for raw_id, data in sorted(
            ...
            if q.include_details:
                ...
                elif boundary is not PlanetBidsAccessState.PUBLIC:
    ...
    async def _get(self, url, params=None):
        if self._circuit_open():
            raise PlanetBidsError("profile circuit is open")
        safe = self._safe_url(url)
        if not safe:
            raise PlanetBidsError("unsafe URL")
        for attempt in range(self.profile.retries + 1):
            try:
                ...
                if response.status_code in {401, 403}:
                    raise PlanetBidsAccessError(PlanetBidsAccessState.LOGIN_REQUIRED)
                response.raise_for_status()
                self._failures = 0
                self._access = PlanetBidsAccessState.PUBLIC
                return response
            except PlanetBidsAccessError:
                raise
            except (httpx.HTTPError, OSError) as exc:
                self._failures += 1
                self._last_failure = self._now()
                self._last_error = type(exc).__name__
                self._access = PlanetBidsAccessState.TRANSIENT_ERROR
                if attempt >= self.profile.retries:
                    raise PlanetBidsError(str(exc)) from exc
