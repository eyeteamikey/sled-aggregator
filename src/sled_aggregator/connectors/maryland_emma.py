"""Bounded, GET-only connector for Maryland's public eMMA pages.

The parser understands only fixture-proven eMMA ``page.aspx`` markup.  It does
not provide generic ASP.NET form automation and never enters response workflows.
"""
...
CANONICAL_FAMILY = "maryland/emma"
PLATFORM_HOSTS = frozenset({"emma.maryland.gov", "procurement.maryland.gov"})


class MarylandEMMAAccessState(StrEnum):
    PUBLIC = "public"
...
class MarylandEMMAError(RuntimeError):
    pass


class MarylandEMMAAccessError(MarylandEMMAError):
    def __init__(self, state: MarylandEMMAAccessState):
        self.state = state
        super().__init__(f"MarylandEMMA access boundary: {state.value}")


@dataclass(frozen=True, slots=True)
class MarylandEMMAProfile:
    profile_key: str
    jurisdiction: str
    state_code: str
    ...
    agency_page_url: str = ""
    discovery_url: str = ""
    supported_hostname: str = "emma.maryland.gov"
...
FIXTURE_PROFILE = MarylandEMMAProfile(
    profile_key="maryland-statewide",
    jurisdiction="Maryland",
    state_code="MD",
    government_level="state",
    agency_name="Maryland State and participating public bodies",
    agency_slug="statewide",
    agency_page_url="https://procurement.maryland.gov/",
    discovery_url="https://emma.maryland.gov/page.aspx/en/rfp/request_browse_public",
    detail_url_template="https://emma.maryland.gov/page.aspx/en/bpm/process_manage_extranet/{opportunity_id}",
    public_notice_url_template="https://emma.maryland.gov/page.aspx/en/bpm/process_manage_extranet/{opportunity_id}",
    public_contract_url="https://emma.maryland.gov/page.aspx/en/ctr/contract_browse_public",
    profile_status="active",
    verification_status="fixture_verified",
    approved_hosts=("emma.maryland.gov",),
...
EMMA_PROFILES = {FIXTURE_PROFILE.profile_key: FIXTURE_PROFILE}


@dataclass(frozen=True, slots=True)
class MarylandEMMAQuery(ConnectorQuery):
...
def detect_access_boundary(text: str) -> MarylandEMMAAccessState:
    value = re.sub(r"\s+", " ", text).casefold()
    for state, terms in (
        (
            MarylandEMMAAccessState.CAPTCHA_REQUIRED,
            ("captcha", "g-recaptcha", "hcaptcha", "challenge-form"),
        ),
        (
            MarylandEMMAAccessState.VENDOR_PROFILE_REQUIRED,
            ("add to my solicitations", "acknowledge receipt", "vendor profile required"),
        ),
        (
            MarylandEMMAAccessState.INVITATION_ONLY,
        ...
        (
            MarylandEMMAAccessState.REGISTRATION_REQUIRED,
            ("registration required", "create a supplier account", "register to download"),
        ),
        (
            MarylandEMMAAccessState.LOGIN_REQUIRED,
            ("login required", "sign in to continue", "supplier login", "maryland sso", "mdot sso"),
        ),
        (MarylandEMMAAccessState.RESTRICTED, ("access denied", "not authorized")),
        (MarylandEMMAAccessState.UNAVAILABLE, ("temporarily unavailable", "scheduled maintenance")),
    ):
        if any(term in value for term in terms):
            return state
    return MarylandEMMAAccessState.PUBLIC
...
def external_platform(url: str) -> str | None:
    ...
    if "bonfirehub" in host:
        return "bonfire"
    if "bidlocker" in host:
        return "bid-locker"
    return "agency-or-other-public-portal" if host else None
...
class MarylandEMMAConnector(BaseConnector):
    ...
    def __init__(
        self,
        profile: MarylandEMMAProfile = FIXTURE_PROFILE,
        *,
        ...
        self._access = MarylandEMMAAccessState.UNKNOWN
    ...
    async def discover(self, query: ConnectorQuery) -> AsyncIterator[RawOpportunity]:
        q = (
            query
            if isinstance(query, MarylandEMMAQuery)
            else MarylandEMMAQuery(
        ...
        if self.profile.profile_status in {"migrated", "legacy", "unavailable"}:
            raise MarylandEMMAAccessError(
                MarylandEMMAAccessState.MIGRATED
                if self.profile.profile_status != "unavailable"
                else MarylandEMMAAccessState.UNAVAILABLE
            )
        if not self._configuration_valid():
            raise MarylandEMMAError("invalid or unsafe MarylandEMMA profile")
        ...
        for page in range(1, pages + 1):
            ...
            if boundary is not MarylandEMMAAccessState.PUBLIC:
                raise MarylandEMMAAccessError(boundary)
            items, links = parse_page(response.text)
            if not items:
                if page == 1 and not re.search(
                    r"no (?:open )?(?:opportunities|solicitations)|data-emma-list",
                    response.text,
                    re.I,
                ):
                    raise MarylandEMMAAccessError(MarylandEMMAAccessState.CHANGED_MARKUP)
                break
            for data in items:
                raw_id = (
        ...
        for raw_id, data in sorted(
            ...
            if q.include_details:
                ...
                elif boundary is not MarylandEMMAAccessState.PUBLIC:
        ...
        return RawOpportunity(
            ...
            raw_payload={
                **data,
                ...
                or ("response_system_external" if data.get("alternateResponseUrl") else "public"),
                ...
                "unspsc_codes": data.get("unspscCodes", []),
                "socioeconomic_designations": data.get("socioeconomicDesignations", []),
                ...
                "alternate_platform": data.get("alternatePlatform")
    ...
    async def _get(self, url, params=None):
        if self._circuit_open():
            raise MarylandEMMAError("profile circuit is open")
        safe = self._safe_url(url)
        if not safe:
            raise MarylandEMMAError("unsafe URL")
        for attempt in range(self.profile.retries + 1):
            try:
                ...
                if response.status_code in {401, 403}:
                    raise MarylandEMMAAccessError(MarylandEMMAAccessState.LOGIN_REQUIRED)
                response.raise_for_status()
                self._failures = 0
                self._access = MarylandEMMAAccessState.PUBLIC
                return response
            except MarylandEMMAAccessError:
                raise
            except (httpx.HTTPError, OSError) as exc:
                self._failures += 1
                self._last_failure = self._now()
                self._last_error = type(exc).__name__
                self._access = MarylandEMMAAccessState.TRANSIENT_ERROR
                if attempt >= self.profile.retries:
                    raise MarylandEMMAError(str(exc)) from exc
    ...
    def _matches(self, item, q):
        ...
        return (
            ...
            and (not q.unspsc or q.unspsc in item.raw_payload.get("unspscCodes", []))
            and (
                not q.certification
                or q.certification.casefold()
                in {
                    str(x).casefold() for x in item.raw_payload.get("socioeconomicDesignations", [])
                }
            )
